...
Article: [
    ...
    Source: [
        ...
        .text: {[http://mycodesnippets.space/redlang/repend.red](https://github.com/lepinekong/mycodesnippets/blob/master/redlang/repend.red)
        }
        .Published-Url: http://mycodesnippets.space/redlang/repend
    ]  
    ...
    demo: [
        ...
        .quote: {do read http://mycodesnippets.space/redlang/src/repend.red}
    ]      
    
    code-snippet: [

        .title: {Code Snippet}
        .image: https://i.imgur.com/kjeZ6e7.png
        .links: [
            {View source} https://github.com/lepinekong/mycodesnippets/blob/master/redlang/src/repend.red
            {Copy source} https://raw.githubusercontent.com/lepinekong/mycodesnippets/master/redlang/src/repend.red
        ]        
        .code/redcode: {
Red [
    Title: "repend.red"
...
while [block0/1 = ""][remove block0] ; remove newlines at the top if any
block: copy [] 
foreach element block0 [
    append block trim/lines trim/head trim/tail element; clean spaces
]
cryptocurrencies: copy []
foreach [name symbol description] block [
    ;instead of using
    ;append/only stocks reduce [symbol name description]
    ;use repend as shortcut for append ... reduce
    repend/only cryptocurrencies [symbol name description]
]

write-clipboard mold cryptocurrencies
